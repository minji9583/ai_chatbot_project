...
def Model(features, labels, mode, params):
    ...
    if params['embedding'] == True:
        ...
        embedding = tf.get_variable(name="embedding_encoder",  # 이름
    ...
    # 임베딩된 인코딩 배치를 생성
    embedding_encoder_batch = tf.nn.embedding_lookup(params=embedding, ids=features['input'])
            ...
            if TRAIN:
                if i > 0:
                    ...
                    input_token_emb = tf.cond(
                        ...
                        lambda: tf.nn.embedding_lookup(embedding, labels[:, i-1]),  # labels 정답을 넣어주고 있다.
                        lambda: tf.nn.embedding_lookup(embedding, output_token) # 모델이 정답이라고 생각 하는 값
                    )
                else:
                    input_token_emb = tf.nn.embedding_lookup(embedding, output_token) # 모델이 정답이라고 생각 하는 값
