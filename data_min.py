...
def load_data():
    ...
    train_q, train_a, test_q, test_a = train_test_split(question, answer)
    ...
    for seq in value:
        ...
        for word in seq.split():
            if dictionary.get(word) is not None:
                ...
                seq_index.extend([int(dictionary[word])])
            else:
        ...
        seq_index += (DEFINES.max_sequence_length - len(seq_index)) * [PAD_INDEX]
...
# print(pred_next_string(["날이 좋아서, 날이 나빠서", "그대가 보고싶네요.", "날이 좋은 날에는, 그대가 보고싶네요."], {'날이': 1, '좋아서': 2, '나빠서': 3, '그대가': 4, '보고싶네요': 5}))
def main(self):
